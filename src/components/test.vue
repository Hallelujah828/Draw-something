      ...
      <div id="control" class="fl">
        ...
        <div id="canvas-color">
          <h5>
            画笔颜色</h5>
          <ul>
            <li v-for="(item,index) in colors" :key="index" :class="{'active':config.lineColor === item}" :style="{ background: item }" @click="setColor(item)"></li>
          </ul>
        </div>
        <!--画笔-->
        <div id="canvas-brush">
          <h5>画笔大小</h5>
          <span v-for="(pen,index) in brushs" :key="index" :class="[pen.className,{'active': config.lineWidth === pen.lineWidth}]" @click="setBrush(pen.lineWidth)"></span>
        </div>
        <!--操作-->
        <div id="canvas-control">
          <h5>操作</h5>
          <span v-for="(control,index) in controls" :key="index" :title="control.title" :class="control.className" @click="controlCanvas(control.action)"></span>
        </div>
        <!-- 生成图像-->
